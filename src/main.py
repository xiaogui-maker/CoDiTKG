...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name,
         static_graph, time_list, history_time_nogt, mode):
...
def run_experiment(args, history_len=None, n_layers=None, dropout=None, n_bases=None, angle=None, history_rate=None):
    ...
    model = RecurrentRGCN(args.decoder,
                          ...
                          # 🆕 归纳式扩散参数
                          use_inductive_diffusion=args.use_inductive_diffusion,
                          diffusion_timesteps=args.diffusion_timesteps,
                          diffusion_sampling_steps=args.diffusion_sampling_steps,
                          diffusion_noise_level_seen=args.diffusion_noise_level_seen,
                          diffusion_noise_level_unseen=args.diffusion_noise_level_unseen,
                          ood_loss_weight=args.ood_loss_weight,
                          diffusion_loss_weight=args.diffusion_loss_weight,
                          distribution_loss_weight=args.distribution_loss_weight,
                          use_meta_learning=args.use_meta_learning,
                          meta_lr=args.meta_lr,
                          use_distribution_alignment=args.use_distribution_alignment,
                          alignment_strength=args.alignment_strength,
                          # 霍克斯参数
                          use_time_decay_gru=args.use_time_decay_gru,
                          use_multikernel_hawkes=args.use_multikernel_hawkes,
                          gru_beta=args.gru_beta,
                          gru_beta_fast=args.gru_beta_fast,
                          gru_beta_slow=args.gru_beta_slow)

    if use_cuda:
        torch.cuda.set_device(args.gpu)
        model.cuda()

    # 🆕 注入训练集实体ID，推理时精确识别未见实体
    if args.use_inductive_diffusion:
        _seen_ent_ids = set()
        for _snap in train_list:
            for _triple in _snap:
                _seen_ent_ids.add(int(_triple[0]))
                _seen_ent_ids.add(int(_triple[2]))
        model.set_seen_entity_ids(_seen_ent_ids)

    if args.add_static_graph:
        static_graph = build_sub_graph(len(static_node_id), num_static_rels, static_triples, use_cuda, args.gpu)

    # ── ✅ 改进5：双优化器分离训练 ──────────────────────────────
    if args.use_inductive_diffusion and hasattr(model, 'inductive_diffusion'):
        main_params = [p for n, p in model.named_parameters()
                       if 'inductive_diffusion' not in n]
        diff_params = list(model.inductive_diffusion.parameters())
        optimizer = torch.optim.Adam(main_params, lr=args.lr, weight_decay=1e-5)
        diff_optimizer = torch.optim.Adam(diff_params, lr=args.lr * 0.1, weight_decay=1e-5)
        print(f"[DiffuTKG] 双优化器启用: 主干参数={len(main_params)}, 扩散参数={len(diff_params)}")
    else:
    ...
    if args.test and os.path.exists(model_state_file):
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                loss_e, loss_r, loss_static, loss_diff = model.get_loss(history_glist, output[0], static_graph,
                                                                        one_hot_tail_seq, one_hot_rel_seq, use_cuda)

                # ── 主任务反向传播（不含扩散损失，保持TiRGN原始训练质量）──
                ...
                # ── 扩散损失独立反向传播（只更新扩散模块自身参数）──────────
                if diff_optimizer is not None and loss_diff.item() > 0:
                    losses_diffusion.append(loss_diff.item())
                    loss_diff.backward()
                    if hasattr(model, 'inductive_diffusion'):
                        torch.nn.utils.clip_grad_norm_(
                            model.inductive_diffusion.parameters(), args.grad_norm
                        )
                    diff_optimizer.step()
                    diff_optimizer.zero_grad()
                else:
                    losses_diffusion.append(0.0)
